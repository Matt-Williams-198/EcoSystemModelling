...
PreyGrowthRates = [2, 3];
PredatorGrowthRates = [0.02, 0.01 ; 0.01, 0.01];%;  0.02, 0.02];
RemovalInterval = 2;
RemovalProportions = 0.1;
Predation1Rates = [0.5, 0.4];%, 0.05];
Predation2Rates = [0.3, 0.2];% 0.05];
PreyCarryingCapacities = [200, 100];
PreyCompetitionCoefficients = [0.01, 0.02];
PredatorCompetitionCoefficients = [0.0, 0.2];%, 0.2];
PredatorDeathRates = [0.1, 0.1];%, 0.1];
TimeEvolvedPopulations = zeros(4,FinalTime);
...
ylabel('Population/Carrying Capacity')%','Tigers'
legend('Sheep','Rabbits','Foxes','Wolves')
